---
title: "Association between UC-PRS and UC phenotype in UK Biobank"
output: html_notebook
Author: "Pooja Middha"
Date: "Oct 12, 2022"
---

Aim: This script has following aims: 
1) Check the distribution of UC-PRS in UC cases and controls in UKBB
2) Compare mean UC-PRS between UC cases and controls
3) Run association analysis between UC-PRS and phenotype in UKBB 
...
Load UC PRS scores calculated by Plink for individuals 

```{r}
# PRS data 
uk_uc_prs <- fread("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/uc_ukb_prs.sscore", header = T)
colnames(uk_uc_prs)[1] <- "FID"
```
...
uk_uc_phen <- subset(final, final$IID %in% uk_uc_prs$IID)
rm(final)
```
...
```{r}
# Merge genetic and phenotype data
ukb_uc <- merge(uk_uc_phen, uk_uc_prs, by.x = c("FID","IID"), by.y = c("FID","IID"))
```

Plot distribution of PRS in cases and controls

```{r}
# Separate UC cases and controls
uc_case <- subset(ukb_uc, ukb_uc$UC == 1)
uc_control <- subset(ukb_uc, ukb_uc$UC == 0)

# Compute mean PRS for cases and controls
case_mean_prs <- uc_case %>% pull(SCORE1_SUM) %>% mean() %>% signif(6)
control_mean_prs <- uc_control %>% pull(SCORE1_SUM) %>% mean() %>% signif(6)

# Plot PRS distribution in cases and controls in UKBB 
a <- ggplot(ukb_uc, aes(x=SCORE1_SUM, fill = as.factor(UC), color = as.factor(UC))) +
            geom_density() + 
            ...
            labs(title = "UC-PRS distribution in UKBB", 
                 x = "UC-PRS",
                 y = "Density", 
...
b <- ggviolin(ukb_uc, x = "UC", y = "SCORE1_SUM", fill = "UC",
              ylab = "UC-PRS", xlab = "Case-Control status", 
...
# Export the plot 
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/UC_PRS_distribution_and_meanPRS_plot.jpeg", width=4450, height=(2800), res=350)
d
dev.off()
```

Association analysis of UC-PRS and UC phenotype (UNADJUSTED MODEL)

```{r}
# Logistic regression (UNADJUSTED)
unadj <- glm(UC ~ SCORE1_SUM, data = ukb_uc, family = "binomial")
summary(unadj)
unadj_df <- NULL
...
write.table(unadj_df, file = "~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/unadjustedprs_logistic_regression.txt", 
            sep = "\t", quote = F, row.names = F)
```

Association analysis of UC-PRS and UC phenotype (ADJUSTED MODEL - AGE, BIOLOGICAL GENDER, PCs, GENOTYPING ARRAY)

```{r}
# Logistic regression (UNADJUSTED)
adj <- glm(UC ~ SCORE1_SUM + UC_agedx_enrol + sex_self + genotyping.array + PC1 + 
                PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                data = ukb_uc, family = "binomial")
summary(adj)
adj_df <- NULL
...
write.table(adj_df, file = "~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/adjustedprs_logistic_regression.txt", 
            sep = "\t", quote = F, row.names = F)
```
...
# Export the plot
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/plot_adjusted_unadjusted_regression_OR.jpeg")
or_p
dev.off()
```
...
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/auc_ucprs_ukbb.jpeg")
rocobj <- plot.roc(ukb_uc$UC, ukb_uc$SCORE1_SUM,
                   main = "AUC-UC", 
                   percent=FALSE,
...
# STANDARDIZED UC-PRS AND RELATED ANALYSIS
Standardize PRS by unit standard deviation of controls

```{r}
# Standard deviation of UC-PRS in contorls
#sd_uc <- as.numeric(sd(uc_control$SCORE1_SUM)) # 1.025009

# Divide PRS of all individuals by SD of PRS from controls
#ukb_uc$sPRS <- as.numeric(ukb_uc$SCORE1_SUM/sd_uc)
#uc_case$sPRS <- as.numeric(uc_case$SCORE1_SUM/sd_uc)
#uc_control$sPRS <- as.numeric(uc_control$SCORE1_SUM/sd_uc)
```
...
ukb_uc$sPRS <- as.numeric((ukb_uc$SCORE1_SUM - mean(uc_control$SCORE1_SUM))/sd(uc_control$SCORE1_SUM))
```
...
#case_mean_sprs <- uc_case %>% pull(sPRS) %>% mean() %>% signif(6)
#control_mean_sprs <- uc_control %>% pull(sPRS) %>% mean() %>% signif(6)

# Plot PRS distribution in cases and controls in UKBB 
sa <- ggplot(ukb_uc, aes(x=sPRS, fill = as.factor(UC), color = as.factor(UC))) +
            geom_density() + 
...
#                      colour="blue", angle = 0, hjust = 1.2) +
            labs(title = "sUC-PRS distribution in UKBB", 
                 x = "sUC-PRS",
                 y = "Density", 
                 color = "Case-control status") + 
            scale_y_continuous(expand = expansion (add = 0.004))
...
# Plot mean PRS and confidence intervals and t.test to test differences in means
sb <- ggviolin(ukb_uc, x = "UC", y = "sPRS", fill = "UC",
              ylab = "sUC-PRS", xlab = "Case-Control status", 
              add = "boxplot", add.params = list(fill = "white")) + 
...
# Export the plot 
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/UC_standardized_PRS_distribution_and_meanPRS_plot.jpeg", width=4450, height=(2800), res=350)
sd
dev.off()
```

Association analysis of sUC-PRS and UC phenotype (UNADJUSTED MODEL)

```{r}
# Logistic regression (UNADJUSTED)
sunadj <- glm(UC ~ sPRS, data = ukb_uc, family = "binomial")
summary(sunadj)
sunadj_df <- NULL
...
# Save unadjusted model results 
write.table(sunadj_df, file = "~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/unadjusted_standardizedprs_logistic_regression.txt", sep = "\t", quote = F, row.names = F)
```

Association analysis of sUC-PRS and UC phenotype (ADJUSTED MODEL - AGE, BIOLOGICAL GENDER, PCs, GENOTYPING ARRAY)

```{r}
# Logistic regression (UNADJUSTED)
sadj <- glm(UC ~ sPRS + UC_agedx_enrol + sex_self + genotyping.array + PC1 + 
                 PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                 data = ukb_uc, family = "binomial")
summary(sadj)
sadj_df <- NULL
...
# Save the adjusted model results
write.table(sadj_df, file = "~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/adjusted_standardizedprs_logistic_regression.txt", sep = "\t", quote = F, row.names = F)
```
...
# Export the plot
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/plot_adjusted_unadjusted_regression_standardized_OR.jpeg")
sor_p
dev.off()
```
...
```{r}
# Plot AUROC of actual PRS in cases and controls together
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/auc_standardizeducprs_ukbb.jpeg")
rocobj <- plot.roc(ukb_uc$UC, ukb_uc$SCORE1_SUM,
                   main = "AUC-UC", 
                   percent=FALSE,
                   ci = TRUE,                  # compute AUC (of AUC by default)
...
# Export the plot
jpeg("~/Box Sync/Proj_GeRI_GI-irAEs/PRS_UKBB/Results/UC/10122022/prs_score/plot_adjusted_unadjusted_standardized_nonstandardized_OR.jpeg", width = 4450, height = 2800, res = 350)
all_or_p
dev.off()
```
