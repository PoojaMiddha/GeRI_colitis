---
title: "HR association between UC-PRS and colitis adverse event in GeRI cohort"
output: html_notebook
Author: "Pooja Middha"
Date: "Feb 15, 2023"
---

Aim: Run Cox proportional hazard analysis between UC-PRS and GI adverse event phenotype in GeRI
...
# PRS data 
prs <- fread(paste0(proj_dir, "Results/UC/geri_uc_prs.txt"))
...
Cox proportional hazards model of UC-PRS and GI-irAE (UNADJUSTED MODEL)
...
write.csv(unadj_cox_df, file = paste0(proj_dir, "Results/UC/unadjusted_PRS_cox_regression.csv"), 
          row.names = F)
```
Cox proportional hazards model of UC-PRS and GI-irAE (ADJUSTED MODEL - AGE, BIOLOGICAL GENDER, PCs)
...
# Save unadjusted cox model results 
write.csv(adj_cox_df, file = paste0(proj_dir, "Results/UC/adjusted_PRS_cox_regression.csv"), 
          row.names = F)
```
...
# Export the plot
jpeg(paste0(proj_dir, "Results/UC/HRplot_adjusted_unadjusted_cox_regression.jpeg"), width=4450, height=(2800), res=350)
hr_p
dev.off()
```
...
Cox proportional hazards model of sUC-PRS and GI-irAE (UNADJUSTED MODEL)
...
# Save unadjusted cox model results 
write.csv(sunadj_cox_df, file = paste0(proj_dir, "Results/UC/unadjusted_sPRS_cox_regression.csv"), 
          row.names = F)
```
Cox proportional hazards model of sUC-PRS and GI-irAE (ADJUSTED MODEL - AGE, BIOLOGICAL GENDER, PCs)
...
# Save unadjusted cox model results 
write.csv(sadj_cox_df, file = paste0(proj_dir, "Results/UC/adjusted_sPRS_cox_regression.csv"), 
          row.names = F)
```
...
# Export the plot
jpeg(paste0(proj_dir, "Results/UC/HRplot_adjusted_unadjusted_cox_regression_standardized_PRS.jpeg"), width=4450, height=(2800), res=350)
shr_p
dev.off()
```
...
# Export the plot
jpeg(paste0(proj_dir, "Results/UC/HRplot_adjusted_unadjusted_standardized_nonstandardized_PRS.jpeg"), width = 4450, height = 2800, res = 350)
all_hr_p
dev.off()
```