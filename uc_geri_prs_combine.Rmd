...
Aim: Combine files obtained from plink (a0 and a1 and per chromosome) to create a composite UC-PRS score for each GeRI participant to run downstream analysis 
...
chr1 <- fread(paste0(projdir, "PRS_GeRI/Results/UC/uc_geri_prs_a0_chr1.sscore"))
colnames(chr1)[1] <- "FID"
...
for (i in 2:22) {
  a0_a1 <- fread(paste0(projdir, "PRS_GeRI/Results/UC/uc_geri_prs_a0_chr", i, ".sscore"))
...
chr_a0 <- chr %>% dplyr::select(IID, contains("SCORE1_SUM_"))
rm(chr)
```
...
```{r}
# Import all chromosomes with a0_a1 suffix and combine all scores into one file 
achr1 <- fread(paste0(projdir, "PRS_GeRI/Results/UC/uc_geri_prs_a1_chr1.sscore"))
...
for (i in 2:22) {
  a1_a0 <- fread(paste0(projdir, "PRS_GeRI/Results/UC/uc_geri_prs_a1_chr", i, ".sscore"))
...
# Final file with IID and scores from each chromosomes
chr_a1 <- achr %>% dplyr::select(IID, contains("SCORE1_SUM_"))
rm(achr)
```

Final step to get UC-PRS for GeRI cohort participants: Add all these per chromosome (a0/a1) scores
...
prsdat <- combdat %>% dplyr::select(IID, prs)
write.table(prsdat, file = paste0(projdir, "PRS_GeRI/Results/UC/geri_uc_prs.txt"), quote = F, sep = "\t", row.names = F)
```
